...
一个功能完备、高可扩展的物联网平台，用最少的代码接入设备，基于`Ruoyi-vue`框架，支持`Mysql`和`pgsql`(企业版)双版本，集成mybatis-plus，集成TCP、MQTT、UDP、CoAP、HTTP、WebSocket、MODBUS等多种网络组件，提供完整的设备接入、管理和数据处理解决方案。具备强大的消息解析、数据转发、规则编排和实时告警能力，帮助企业快速构建物联网应用。

#### 该仓库代码为社区版，将会不定时更新，文档末尾有社区和`企业版`本功能对比，企业版可通过`文档末尾`联系
